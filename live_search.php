...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['search'])) {
        ...
        try {
            ...
            if ($connection) {
                // SQL query with JOIN
                $query = $connection->prepare("SELECT users.User_Id, users.firstname, users.lastname, users.birthday, users.sex, username, users.user_profile_picture, CONCAT(user_address.city,', ', user_address.province) AS address FROM users INNER JOIN user_address ON users.User_Id = user_address.User_Id WHERE users.username LIKE ? OR users.firstname LIKE ? OR users.User_Id LIKE ? OR CONCAT(user_address.city,', ', user_address.province) LIKE ? ");
